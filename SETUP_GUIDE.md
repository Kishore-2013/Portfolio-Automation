...
I have added several helper scripts to the root `package.json` to simplify management:
- `npm run dev`: Starts both the web frontend and server API concurrently.
- `npm run install-all`: Installs dependencies for the root, web, and server1.
- `npm run web`: Starts only the Next.js dashboard.
- `npm run server`: Starts only the Express API.

## 4. Testing with Mock Data
If you don't have a resume PDF ready, you can use the `mock_portfolio_data.json` file I created in the root directory. This matches the schema required by the portfolio templates.
