...
import { Github, Target, Eye, EyeOff, Lock, Mail, ArrowRight, Shield } from "lucide-react"
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (accessToken) {
                ...
                enqueueSnackbar("Synchronization successful.", { variant: "success", style: { background: "#10b981", color: "white", borderRadius: "12px", fontFamily: "monospace", fontSize: "12px", textTransform: "uppercase" } })
                ...
                <div className="relative p-8 md:p-10 bg-black/40 backdrop-blur-3xl rounded-[2.4rem] border border-white/5 shadow-2xl space-y-8 z-10 transition-colors duration-500 hover:bg-black/50">
                    ...
                    <div className="text-center space-y-2 relative z-10">
                        <div className="flex items-center justify-center gap-2 mb-4">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-[0.5em] text-primary italic">Identity Verification</span>
                            <div className="h-px w-8 bg-primary/20" />
                        </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
                        <div className="space-y-2">
                            <label className="text-[10px] font-black text-gray-500 uppercase tracking-[0.3em] flex items-center gap-2">
                                <Mail className="w-3 h-3 text-primary" /> Email Identity
                            </label>
                            <div className="relative group">
                                <Input
                                    ...
                                    className="h-14 md:h-16 px-6 rounded-2xl bg-white/[0.03] border-white/5 transition-all focus:border-primary/50 focus:bg-white/[0.05] font-mono text-sm placeholder:opacity-20 uppercase tracking-widest"
                                    required
                                    disabled={isLoading}
                                />
                            </div>
                        ...
                        <div className="space-y-2 relative">
                            <div className="flex justify-between items-center">
                                <label className="text-[10px] font-black text-gray-500 uppercase tracking-[0.3em] flex items-center gap-2">
                                    <Lock className="w-3 h-3 text-primary" /> Access Key
                                </label>
                                <Link href="#" className="text-[9px] font-black uppercase tracking-widest text-primary/60 hover:text-primary transition-colors italic">Lost Key?</Link>
                            </div>
                            <div className="relative group">
                                <Input
                                    ...
                                    className="h-14 md:h-16 px-6 rounded-2xl bg-white/[0.03] border-white/5 transition-all focus:border-primary/50 focus:bg-white/[0.05] font-mono text-sm placeholder:opacity-20"
                                    required
                                    disabled={isLoading}
                                />
                                <button
                        ...
                        <Button 
                            type="submit" 
                            disabled={isLoading}
                            className="w-full h-16 md:h-20 rounded-2xl bg-primary text-white text-base font-black uppercase italic tracking-[0.2em] transition-all hover:scale-[1.02] active:scale-95 shadow-[0_10px_40px_rgba(37,99,235,0.3)] group overflow-hidden relative"
                        >
                    ...
                    <div className="text-center pt-4 relative z-10">
                        <p className="text-[10px] text-gray-500 font-black uppercase tracking-[0.2em]">
                            New to the matrix? <Link href="/register" className="text-primary underline decoration-primary/30 underline-offset-4 hover:decoration-primary transition-all">Create Identity</Link>
                        </p>
                    </div>
                </div>
            </div>
