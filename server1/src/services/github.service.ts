...
// ── Get Repo ID ──────────────────────────────────────────────────────────
export const getRepoId = async (repoName: string, ownerOverride?: string): Promise<number> => {
  const owner = ownerOverride || getGitHubOwner();
  try {
    const response = await githubClient.get(`/repos/${owner}/${repoName}`);
    return response.data.id;
  } catch (err: any) {
    throw new Error(`Could not find GitHub repository ID for ${owner}/${repoName}: ${err.message}`);
  }
}
