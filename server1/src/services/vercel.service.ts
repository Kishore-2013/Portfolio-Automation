  ...
  const payload: Record<string, unknown> = {
    ...
    projectSettings: {
      framework: (
        ...
        fs.existsSync(path.join(env.PROJECTS_BASE_PATH!, repoName, 'next.config.js')) ||
        fs.existsSync(path.join(env.PROJECTS_BASE_PATH!, repoName, 'next.config.ts')) ||
        fs.existsSync(path.join(env.PROJECTS_BASE_PATH!, repoName, 'web', 'next.config.js')) ||
        fs.existsSync(path.join(env.PROJECTS_BASE_PATH!, repoName, 'web', 'next.config.ts'))
      ) ? 'nextjs' : 'vite',
    },
  }
...
export const waitForDeployment = async (
  ...
  for (let attempt = 0; attempt < 30; attempt++) {
    ...
    const res = await vercelClient.get(`/v13/deployments/${deploymentId}`, { params })
